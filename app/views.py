...
class ResidenceViewSet(viewsets.ModelViewSet):
    ...
    pagination_class = MyPagination         # Pagination Added Manually
...
class CheckLoginValidity(APIView):
    def get(self, request):
        ...
        # request_code = self.request.query_params.get('code', None)
        # if request_code:
        #     queryset = Resident.objects.all().filter(passcode=request_code)

        # residence_area_code = self.request.query_params.get('area_code', None)
        # if residence_area_code:
        #     queryset = queryset.filter(residence_area__residence=residence_area_code)
...
class VisitorsByResidence(viewsets.ModelViewSet):
    queryset = Visitor.objects.all()
    serializer_class = VisitorSerializer
...
class AppUserViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        someset = AppUser.objects.all()
        someset = someset.filter(name__icontains=name) if name else someset
        someset = someset.filter(phone__icontains=phone) if phone else someset 
        someset = someset.filter(email__icontains=email) if email else someset
        someset = someset.filter(passcode__icontains=passcode) if passcode else someset
        someset = someset.filter(usertype__app_user_type__icontains=user_type) if user_type else someset
        return someset
...
class OnesignalNotification(APIView):
    queryset = AppUser.objects.all()
    serializer_class = AppUserSerializer

    def post(self, request, format=None):
        appuseremail     = self.request.data.get('email', -1)
        appuserpasscode  = self.request.data.get('passcode', -1)
        appuserbrowserid = self.request.data.get('browser_id')
        
        someset = AppUser.objects.all().filter(email=appuseremail, passcode=appuserpasscode)

        data = { 'browser_id' : appuserbrowserid}
        s = AppUserSerializer(someset.first(), data=data, partial=True, context={'request': request})
        if s.is_valid():
            s.save()
...
class AgentByPasscodeAndArea(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        agentUserType = ""
        for user in appUserTypes.objects.all():
            if user.app_user_type == 'Security Agent':
                print(user.app_user_type)
                agentUserType = user.app_user_type
                someset = AppUser.objects.all().filter(usertype__app_user_type=agentUserType, passcode=codefor, residence=residencefor)[:1]
                return someset 
        return [] 

class AgentsByResidence(viewsets.ModelViewSet):
    queryset = AppUser.objects.all() 
    serializer_class = AppUserSerializer
    def get_queryset(self):
        residenceAreafor = self.request.query_params.get('residence', -1)
        agentUserType = ""
        for user in appUserTypes.objects.all():
            if user.app_user_type == 'Security Agent':
                print(user.app_user_type)
                agentUserType = user.app_user_type
                someset = AppUser.objects.all().filter(usertype__app_user_type=agentUserType, residence=residenceAreafor)
                return someset 
        return []
...
class ConnectionsViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        request_residence = self.request.query_params.get('phone', None)
        someset = Resident.objects.all().filter(phone=request_residence) 
        return someset
...
class VisitorsByResident(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        someset = Visitor.objects.filter(resident=resident_id) 
        return someset
...
class VisitorsByResidence(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        someset = Visitor.objects.filter(Q(residence_area__residence=residence_id)) 
        
        # someset = Visitor.objects.filter(residence=residence) 
        return someset
